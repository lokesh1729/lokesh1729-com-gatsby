...
const SnippetsTemplate: React.FC<Props> = ({ data, pageContext }: Props) => {
...
export const query = graphql`
  query IndexTemplate($limit: Int!, $offset: Int!) {
    allMarkdownRemark(
      ...
      filter: {
        frontmatter: { template: { eq: "snippet" }, draft: { ne: true } }
      }
    ) {
      edges {
        node {
...
export const Head: React.FC<Props> = ({ pageContext }) => {
  ...
  const pageTitle = page > 0 ? `Snippets - Page ${page} - ${title}` : title;
