...
const BytesTemplate: React.FC<Props> = ({ data, pageContext }: Props) => {
...
export const query = graphql`
  query IndexTemplate($limit: Int!, $offset: Int!) {
    allMarkdownRemark(
      ...
      filter: { frontmatter: { template: { eq: "byte" }, draft: { ne: true } } }
    ) {
      edges {
        node {
          fields {
...
export const Head: React.FC<Props> = ({ pageContext }) => {
  ...
  const pageTitle = page > 0 ? `Bytes - Page ${page} - ${title}` : title;
